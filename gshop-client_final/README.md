...
## 1. 项目开发准备
    项目描述
    1，项目是关于外卖业务的前后台分离Web APP（基于Vue2.5版本的）
    2，功能模块包含商家，商品，购物车，用户管理等多个模块
    3，前台应用技术架构为：vue+vuex+vue-router+webpack+ES6
	4，本项目采用模块化，组件化，工程化（webpack,脚手架）的模式开发。
	5，(后台应用技术架构为：Node+Express+Mongodb+Mongoose)

    技术选型
     （1）适用代理服务器解决跨域
      config/index.js下target: 'http://localhost:4000', // 代理目标的基础路径、
	...
	（3）ES7新语法：async/await
		异步获取地址，异步获取食品分类列表，异步获取商家列表
		发送异步ajax请求，提交一个mutation，根据api接口code，code为0表示成功

	（4）流行的Ajax请求库：axios
	（5）常用的滑动库：better-scroll,swiper
	（6）Vue移动端UI组件库：mint-ui
	（7）常用的图片懒加载插件：vue-lazyload   img v-lazy
	（8）一次性短信验证码和图片验证码技术
